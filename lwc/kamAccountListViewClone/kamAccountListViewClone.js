...
import getAccountRecords from '@salesforce/apex/KAMAccountControllerClone.getAccountRecords';
import cloneRecords from '@salesforce/apex/KAMAccountControllerClone.cloneRecords';
import { ShowToastEvent } from 'lightning/platformShowToastEvent';
import {getRecordNotifyChange} from 'lightning/uiRecordApi';
import { RefreshEvent } from 'lightning/refresh';
import { NavigationMixin } from "lightning/navigation";

export default class KamAccountListViewClone extends NavigationMixin(LightningElement)  {
    @api caRecordId;
    ...
    fetchData() {
        getAccountRecords({recId: this.caRecordId})
            .then(response => {
    ...
    handleSave() {
        if (this.selectedAccounts && this.selectedAccounts.length > 0) {
            ...
            cloneRecords({ recId: this.caRecordId, accId: this.selectedAccounts })
                .then(result => { 

                    console.log('@@Result : '+JSON.stringify(result.aptRecordId));
                    // const notifyChangeIds = result.aptRecordId;
                    ...
                    // when we select one account it will navigate to that APT record Page.
                    if(this.selectedAccounts.length == 1){
                        this[NavigationMixin.Navigate]({
                            type: "standard__recordPage",
                            attributes: {
                              recordId: result.aptRecordId,
