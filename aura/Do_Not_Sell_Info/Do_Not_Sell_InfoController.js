({
    ...
    handleClick : function(component, event, helper) {
        ...
        if(label == 'Submit'){
            var contactFields = component.find("Do_Not_Sell_Info");
            var blank=0;
            ...
            if(blank==0) {
                ...
                var error = false;
                if(!component.get("v.CaseValues.Do_not_Sell_my_data__c")){
                    component.set("v.showSpinner",false);
                    helper.showToast(component, event, helper,
                                     'Please check Do Not Sell checkbox to continue','Required!');
                    error = true;
                }
                else if(verifyCaptcha && !error){
